<template>
  <a-card :bordered="false">
    <a-result :sub-title="description" :title="title">
      <template #icon>
        <a-icon type="meh" theme="twoTone" two-tone-color="#E6A23C" />
      </template>
      <template #extra>
...
export default {
  ...
  methods: {
    ...
    refresh() {
      ...
      this.$http({
        method: 'get',
        url: '/api/a/requestInfo/findByUuId?uuId=' + this.requestData.uuId,
      })
        .then(function (res) {
          console.log(res);
    ...
    //刷新路由
    setRouter() {
      if (this.requestData.requestStatus === 0) {
        this.get1();
      }
      if (this.requestData.requestStatus === 1) {
        this.$router.push({path:'/request/requestDetail/success',query: this.requestData});
      }
      if (this.requestData.requestStatus === 2) {
        this.$router.push({path:'/request/requestDetail/error',query: this.requestData});
      }
    }
